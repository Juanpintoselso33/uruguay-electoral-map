...
- `build-odd-sublemas.py` — **post-step: inyecta el nivel `sublema` en la contienda ODD** del `catalogo.json` de internas (los 19 deptos). El desglose-de-votos NO trae la columna SUBLEMA; sí la **"Integración hojas de votación"** de la Corte (`hoja → sublema` por `TipoHoja=ODD`; 2024 = XLSX hoja `Datos`, 2019 = CSV). Agrega nodos `sublema` (id `{lemaId}-sl-{slug}`, misma convención que nacionales) + `grupoId`/`sublemaId` en cada opción, y sube `niveles` a `["lema","sublema","hoja"]` (degradado `["lema","hoja"]` si el depto no tiene sublemas con dato). Parchea en sitio (preserva los lemaId "bare" del sweep, no toca shards de votos) e idempotente. Uso: `python scripts/build-odd-sublemas.py internas-2024` · `… internas-2019`. **Requiere** que `ESCALERAS` declare `(internas, odd) = ['lema','sublema','hoja']` y debe **re-correrse si se regenera el catálogo de internas** (el aggregator TS no lleía la integración).
