...
- **Acordeón de opción** (`option-accordion`, Epic 10) — the `selector-opcion` field expands into a nested **multi-select** disclosure tree (contienda → lema → precandidato/sublema → lista). Each row has three zones: a **tri-state checkbox** (empty / partial `–` / checked) in `{colors.ink}`, a leading **chevron** in `{colors.ink-soft}` (`▸`/`▼`) for rows with children, the node label in `{typography.body}` `{colors.ink}` with a left **indent step per depth level** (one `{spacing}` unit each), and a right-aligned **percent pill** in `{typography.meta}` `{colors.ink-soft}`. Checkbox and chevron are **separate** hit-areas, each ≥44×44px (select vs expand). A checked node carries a 3px left bar in its party data color (lemas/listas) or `{colors.accent}` (nodes without a party color). A **lista** row prefixes its label "Lista " + number in `{typography.body}` and shows lema sigla + precandidato as `{typography.meta}` `{colors.ink-soft}` context. Above the tree sit a `search` field (filter by number) and a `{colors.card}` party filter; below, `active-filters` chips. For flat types (balotaje/plebiscito) the tree renders chevron-less, one level, checkboxes intact. Visual contract only; behavior lives in EXPERIENCE.
- **Conmutador de modo de mapa** (`map-mode-switch`, Epic 10) — a segmented control in the legend naming the active coloring mode: **Ganador · Share % · Votos · Heatmap**. `{typography.label-caps}`, the active segment filled `{colors.ink}` with `{colors.paper}` text, inactive in `{colors.ink-soft}`. The legend below it always restates the mode's unit and current range. ≥44px targets. Only "Ganador" shows with no selection; the other three appear once a selection exists.
