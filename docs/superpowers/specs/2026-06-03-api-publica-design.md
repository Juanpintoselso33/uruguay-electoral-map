...
- **Legisladores (requiere datos nuevos):** el voto legislativo en Uruguay es **a la LISTA, no a la persona** → "votos de un candidato" = votos de su(s) **hoja(s)**. El mapeo **persona↔hoja** existe como dato abierto en el recurso **"Integración de hojas de votación"** de cada dataset (CKAN). Forma real verificada (nacionales-2024, CSV ~28 MB; el `integracion-hojas-de-votacion.csv` del repo era un placeholder vacío): `Numero(hoja), Departamento, PartidoPolitico, Agrupacion, Candidatura(cargo: PRESIDENCIAL/SENADOR/DIPUTADO/…), Sublema, Nombre, CredencialSerie, CredencialNumero, Sexo, Ordinal, TitularSuplente`. **`CredencialSerie+CredencialNumero` es un id de persona estable** → la normalización de identidad cross-elección se resuelve por credencial (no por matching de nombres). Esto es un subsistema de ETL propio → **Epic 21**, que alimenta el recurso `candidatos` para cubrir legisladores.
