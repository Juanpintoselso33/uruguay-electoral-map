/**
 * Orchestrator ETL — departamentos del interior, nacionales-2024 (1ª vuelta), Story 7.4.
 *
 * Reutiliza la geometría serie.topo.json ya generada por el ETL de internas de cada
 * departamento. No regenera geometría; solo agrega votos y corre los gates.
 *
 * Fuente: data/raw/electoral/nacionales-2024/desglose-de-votos.csv
 * Columnas: TipoRegistro, Departamento, CRV, Series, Lema, Descripcion1, CantidadVotos
 *
 * Se procesan todos los 18 departamentos del interior de forma secuencial.
 * Los gates de cobertura usan placementMin configurable por dept (default 0.95).
 */
...
interface NacionalesInteriorConfig {
  /** Código 2 letras en el CSV (e.g., 'AR', 'CA'). */
  deptCode: string;
  /** Nombre en minúsculas con guiones bajos (e.g., 'artigas', 'cerro_largo'). */
  deptName: string;
  /** Serie exterior a excluir (e.g., 'IZZ' para Artigas). */
  exteriorSerie: string;
  /** Umbral mínimo de placement para el gate de cobertura (default 0.95). */
  placementMin?: number;
}

function runNacionales2024Interior(
  cfg: NacionalesInteriorConfig,
  ...
  console.log(`Dependencia: ${GEO_IN} debe existir.`);

  // Filtrar: dept + HOJA_EN|VOTO_LEMA + sin serie exterior
  ...
  console.log(
    `Reconciliación: sum-in=${rec.sumIn.toLocaleString('es-UY')} delta=${rec.delta} ✅`,
  );
  ...
  if (cov.shardSinMatch.length > 0) {
    console.log(
      `⚠️ geoIds sin match en geometría (${cov.shardSinMatch.length}): ${cov.shardSinMatch.join(', ')}`,
    );
  }
...
function main(): void {
  console.log('=== ETL Interior nacionales-2024 — todos los departamentos ===');
  ...
  for (const cfg of DEPTS) {
    try {
      runNacionales2024Interior(cfg, allRows);
      ok++;
    } catch (e) {
  ...
  console.log(`\n=== Interior nacionales-2024 completo: ${ok}/${DEPTS.length} departamentos ===`);
  if (failed.length > 0) {
